...
export const ServicesHero = () => {
  ...
  const backgroundImages = [
    "/images/servicios banner.webp",
    "/images/servicios banner 2.webp",
    "/images/servicios banner 3.webp",
  ];

  const stats = [
    {
      icon: (
        <Star className="w-5 h-5 text-[#F20C1F] group-hover:scale-110 group-hover:rotate-12 transition-transform duration-300" />
      ),
      value: "4.9",
      label: "Rating",
    },
    {
      icon: (
        <Shield className="w-5 h-5 text-[#F20C1F] group-hover:scale-110 group-hover:rotate-12 transition-transform duration-300" />
      ),
      value: "15+",
      label: "Servicios",
    },
    {
      icon: (
        <Users className="w-5 h-5 text-[#F20C1F] group-hover:scale-110 group-hover:rotate-12 transition-transform duration-300" />
      ),
      value: "24/7",
      label: "Atención",
    },
    {
      icon: (
        <Award className="w-5 h-5 text-[#F20C1F] group-hover:scale-110 group-hover:rotate-12 transition-transform duration-300" />
      ),
      value: "100%",
      label: "Confianza",
    },
  ...
  return (
    <section className="relative min-h-screen h-screen max-h-[1200px] flex items-center justify-center text-center overflow-hidden">
      ...
      {/* Animated Service Icons - Responsive */}
      <div className="absolute top-20 right-16 sm:top-32 sm:right-32 opacity-20">
        <div className="animate-bounce delay-200">
          <Wifi className="w-6 h-6 sm:w-8 sm:h-8 text-white/30" />
        </div>
      </div>
      ...
      <div className="absolute bottom-16 right-20 sm:bottom-28 sm:right-40 opacity-20">
        <div
          ...
          style={{ animationDirection: "reverse" }}
        >
          <Shield className="w-5 h-5 sm:w-6 sm:h-6 text-[#F20C1F]/30" />
        </div>
      </div>
      ...
      <div className="relative z-10 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full mt-20 sm:mt-24 lg:mt-32">
        ...
        {/* Main Title - Responsivo progresivo */}
        <motion.h1
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.4, duration: 0.8 }}
          className="text-3xl xs:text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-bold text-white mb-4 sm:mb-6 leading-tight"
        >
        ...
        >
          ...
          <span className="text-[#F20C1F] font-semibold">perfecta</span> y sin{" "}
          <span className="text-[#F20C1F] font-semibold">preocupaciones</span>.
          <br className="hidden sm:block" />
          <span className="text-white/70 text-sm sm:text-base md:text-lg block mt-1 sm:mt-2">
            Servicios diseñados para superar todas tus expectativas
          </span>
        </motion.p>

        {/* Stats - Grid responsivo mejorado */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          {stats.map((stat, index) => (
            <div
              key={index}
              className="flex items-center gap-1 px-3 py-2 sm:px-4 bg-white/10 backdrop-blur-md rounded-lg border border-white/20 hover:bg-white/20 transition-all duration-300 group hover:scale-105 sm:hover:scale-110 hover:rotate-1 cursor-pointer min-w-[90px] sm:min-w-[110px]"
            >
