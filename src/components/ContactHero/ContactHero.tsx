...
export const ContactHero = () => {
  ...
  const backgroundImages = [
    "/images/contantanos banner.webp",
    "/images/contantanos banner 2.webp",
    "/images/contantanos banner 3.webp",
  ];

  const stats = [
    {
      icon: (
        <Phone className="text-[#F20C1F] group-hover:scale-110 group-hover:rotate-12 transition-transform duration-300" />
      ),
      value: "24/7",
      label: "Atención",
    },
    {
      icon: (
        <Mail className="text-[#F20C1F] group-hover:scale-110 group-hover:rotate-12 transition-transform duration-300" />
      ),
      value: "<2h",
      label: "Respuesta",
    },
    {
      icon: (
        <MapPin className="text-[#F20C1F] group-hover:scale-110 group-hover:rotate-12 transition-transform duration-300" />
      ),
      value: "5",
      label: "Oficinas",
    },
  ...
  return (
    <section className="relative min-h-screen h-screen max-h-[1200px] flex items-center justify-center text-center overflow-hidden">
      ...
      {/* Animated Contact Icons - Responsive */}
      <div className="absolute top-20 right-16 sm:top-32 sm:right-32 opacity-20">
        <div className="animate-bounce delay-200">
          <Phone className="w-6 h-6 sm:w-8 sm:h-8 text-white/30" />
        </div>
      </div>
      <div className="absolute bottom-24 left-8 sm:bottom-40 sm:left-20 opacity-20">
        ...
        >
          <Mail className="w-5 h-5 sm:w-6 sm:h-6 text-[#F20C1F]/40" />
        </div>
      </div>
      ...
      <div className="absolute bottom-16 right-20 sm:bottom-28 sm:right-40 opacity-20">
        <div
          ...
          style={{ animationDirection: "reverse" }}
        >
          <Clock className="w-5 h-5 sm:w-6 sm:h-6 text-[#F20C1F]/30" />
        </div>
      </div>
      ...
      <div className="relative z-10 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full mt-20 sm:mt-24 lg:mt-32">
        ...
        >
          {stats.map((stat, index) => (
            <div
              key={index}
              className="flex items-center gap-1 px-1 py-2 sm:px-4 bg-white/10 backdrop-blur-md rounded-lg border border-white/20 hover:bg-white/20 transition-all duration-300 group hover:scale-105 sm:hover:scale-110 hover:rotate-1 cursor-pointer min-w-[90px] sm:min-w-[110px]"
            >
