...
export const RoomFilters = () => {
  ...
  const [iconMap, setIconMap] = useState<{ [key: string]: ReactNode }>({}); // Estado para almacenar iconos
  ...
  // Efecto para cargar iconos de amenidades
  useEffect(() => {
    const loadIcons = async () => {
      const icons: { [key: string]: ReactNode } = {};
      for (const amenity of amenitiesList) {
        icons[amenity.id] = await getAmenityIcon(amenity.icon || "Star");
      }
      setIconMap(icons);
    };

    loadIcons();
  }, [amenitiesList, getAmenityIcon]);

  const handleApplyFilters = () => {
    applyFilters();
    setTimeout(() => {
      console.log(filteredRooms?.length || 0);
    }, 100);
  };
          ...
          <div className="px-6 py-3 bg-[#020659]/5 border-b border-[#020659]/10">
            <div className="flex flex-wrap gap-2">
              ...
              {selectedAmenities.map((amenity) => (
                ...
                >
                  {iconMap[amenity]} {/* Usar el icono cargado o un valor por defecto */}
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
                  {previewAmenities.map((amenity) => (
                    ...
                    >
                      {iconMap[amenity.id] } {/* Usar el icono cargado o un valor por defecto */}
