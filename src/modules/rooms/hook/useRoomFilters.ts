...
export const useRoomFilters = () => {
  ...
  const getViewIcon = (viewName: string) => {
    ...
    if (
      ...
      lowerName.includes("océano") ||
      lowerName.includes("ocean")
    )
      return "🌊";
    if (lowerName.includes("ciudad") || lowerName.includes("city")) return "🏢";
    if (lowerName.includes("jardín") || lowerName.includes("garden"))
      return "🌿";
    if (lowerName.includes("montaña") || lowerName.includes("mountain"))
      return "⛰️";
  ...
  const applyFilters = async (filters?: RoomFiltersType) => {
    ...
    const currentPriceRange =
      filters?.priceRange ??
      (minPrice || maxPrice
        ? { min: minPrice || 0, max: maxPrice || 9999 }
        : undefined);
    const currentAmenities =
      filters?.amenities ??
      (selectedAmenities.length > 0 ? selectedAmenities : undefined);

    const apiFilters: Partial<RoomFiltersType> = {};
    ...
    if (currentAmenities && currentAmenities.length > 0)
      apiFilters.amenities = currentAmenities;
